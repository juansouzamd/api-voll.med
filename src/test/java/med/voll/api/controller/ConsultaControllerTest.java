...
class ConsultaControllerTest {
    ...
    @Test
    @DisplayName("Deve devolver condigo http 400 quando informações estao invalidas")
    @WithMockUser
    ...
    @Test
    @DisplayName("Deve devolver condigo http 200 quando informações estao válidas")
    @WithMockUser
    void agendarCenario2() throws Exception {
        var data = LocalDateTime.now().plusHours(1);
        var especialidade = Especialidade.CARDIOLOGIA;

       var dadosDetalhamento = new DadosDetalhamentoConsulta(null, 17l, 3l, data);
       ...
       when(agendaDeConsultas.agendar(any())).thenReturn(dadosDetalhamento);
        ...
        var response = mvc.perform(post("/consultas")
                        ...
                        .content(dadosAgendamentoConsultaJacksonTester.write(
                                new DadosAgendamentoConsulta(17l, 3l, data, especialidade)
                        ).getJson()))
                .andReturn().getResponse();
